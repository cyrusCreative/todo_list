...
//Adding category and task togrther with JS
...
const renderCategories = () => {
  ...
  categories.forEach((category) => {
    // Get the task of the current category
    const categoryTasks = tasks.filter(
    ...
    div.addEventListener("click", () => {
      ...
      calculateTotal();
      // Re-render tasks when the category change
      renderTasks();
    });
    div.innerHTML = `
        <div class="left">
...
const renderTasks = () => {
  ...
  // if no task for the selected category
  if (categoryTasks.length === 0) {
    ...
    categoryTasks.forEach((task) => {
      ...
      // Add complete functionality on click checkbox
...
// Let add functionalitu to add new tasks
